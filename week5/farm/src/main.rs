...
fn pop_number(numbers_ref: &Arc<Mutex<VecDeque<u32>>>) -> Option<u32> {
    let mut nums = numbers_ref.lock().unwrap();
    if (*nums).len() == 0 {
        return None;
    }
    (*nums).pop_front()
...
fn main() {
    ...
    for i in 0..num_threads {
        let numbers_ref = numbers.clone(); // if we try to use reference to number, maybe thread lives longer than main which makes numbers out of scope and thread now has a dangling pointer
        threads.push(thread::spawn(move || {
            loop {
                if let Some(num) = pop_number(&numbers_ref) { 
                    factor_number(num);
                } else {
                    return;
                }
