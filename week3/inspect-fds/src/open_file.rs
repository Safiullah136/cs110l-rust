...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let fd_string = fs::read_to_string(format!("/proc/{}/fdinfo/{}", pid, fd)).ok()?;
        let cursor = OpenFile::parse_cursor(&fd_string)?;
        let access_mode = OpenFile::parse_access_mode(&fd_string)?;
