...
fn main() {
    ...
    let process_option = ps_utils::get_target(target).expect("ps or pgrep failed.");
    ...
    process.print();
    for child_proc in ps_utils::get_child_processes(process.pid).expect("Failed to get child processes of specified process.") {
        child_proc.print();
    }
}
